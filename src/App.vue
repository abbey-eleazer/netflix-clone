...
<div class="w-full fixed h-screen bg-black">
  <div v-if="!showFullVideo" id="SideNav" class="flex z-40 items-center w-[120px] h-screen bg-black relative">
    <img class="absolute top-0 w-9 mt-10 ml-10" src="/images/netflix-logo.png" alt="logo">
    <div>
  ...
  <div v-if="!showFullVideo">
    <div class="fixed flex z-20 top-0 right-0 w-full h-[50%] bg-black pl-[120px] bg-clip-border">
      <div class="absolute z-30 top-0 right-0 w-[77%] h-[600px] left[120px] bg-gradient-to-r from-black via-black "/>
      <MovieDetail v-if="movie" :movie="movie"  />

      <video 
      v-if="movie" :src="'/videos/'+movie.name+'.mp4'" autoplay loop class="absolute z-0 h-[600px] right-0 top-0"/>
    </div>
  </div>

  <div v-if="!showFullVideo" class="fixed z-30 bottom-0 right-0 w-full  h-[55%] pl-[120px] overflow-y-auto">
    ...
    <div v-if="showFullVideo">
      <div @click="$event => showFullVideo = false" class="absolute z-50 p-2 m-4 bg-white bg-opacity-50 rounded-full cursor-pointer">
        <ChevronLeft fill-color="#fff" :size="40" />
        </div>
        <video 
        v-if="movie" :src="'/videos/'+movie.name+'.mp4'" autoplay loop controls  class="absolute z-0 w-[100vw] h-full object-fit"/>
        </div>
</div>
